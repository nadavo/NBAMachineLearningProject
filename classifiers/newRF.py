...
def my_custom_loss_func(data, labels):
    #diff = np.abs(data - labels).max()
    #return np.log(1 + diff)
    #columns reduced - acc=0.578

    diff = sum((data - labels)**2)+7*np.count_nonzero(data - labels)
    return diff
...
def evaluateModel(clf, data, labels, cv_flag=True):
    if cv_flag:
...
def createModel(data, labels, cv_flag=True):
    ...
    if cv_flag:
        ...
        print('score - ', score(clf, data, labels))
        train_end = time()
