...
def createModel(data, labels, cv_flag=False):
    ...
    if cv_flag:
        ...
        clf = LogisticRegressionCV(solver='sag',max_iter=100000,multi_class='multinomial', class_weight='balanced', Cs=10, cv=StratifiedKFold(n_splits=3, random_state=1),n_jobs=-1,random_state=1)
        errors.append(evaluateModel(clf, data, labels, True))
        plot_learning_curve(clf, "Learning Curves (Logistic Regression)", data, labels, (0.7, 1.01),cv=StratifiedKFold(n_splits=3, random_state=1), n_jobs=-1)
    else:
        ...
        plot_learning_curve(clf, "Learning Curves (Logistic Regression)", X_train, Y_train, (0.7, 1.01),cv=StratifiedKFold(n_splits=3, random_state=1), n_jobs=-1)
    plt.show()
    return errors
