from sklearn.model_selection import train_test_split, learning_curve, StratifiedKFold, cross_val_predict
...
def createModel(data, labels, cv_flag=False):
    errors = list()
    clf = Perceptron(tol=0.00001, class_weight='balanced', n_jobs=-1, random_state=1)
    print("Perceptron")
    if cv_flag:
        print("Cross-Validation")
        errors.append(evaluateModel(clf, data, labels, True))
        plot_learning_curve(clf, "Learning Curves (Perceptron)", data, labels, (0.7, 1.01),cv=StratifiedKFold(n_splits=3, random_state=1), n_jobs=-1)
    else:
        ...
        plot_learning_curve(clf, "Learning Curves (Perceptron)", X_train, Y_train, (0.7, 1.01),cv=StratifiedKFold(n_splits=3, random_state=1), n_jobs=-1)
    plt.show()
    return errors
